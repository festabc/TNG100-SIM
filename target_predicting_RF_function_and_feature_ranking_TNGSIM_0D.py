...
#     pipeline = make_pipeline(RandomForestRegressor())
    ...
    reg_RF = GridSearchCV(model, grid, cv = KFold(n_splits=3, shuffle=True), verbose = 1, n_jobs = -1, return_train_score=True)
    reg_RF.fit(X, y)
...
#     ax.plot([0.0, 150], [0.0, 150], color = 'black', linewidth = 2) # use this scale for TNG100 where the unit of Rdisk is in kpc, while the unit of Rhalo is in Mpc; and, therefore the normalized Halfmass Radius (~Rdisk/Rhalo) has a factor of 10^3
    ax.plot([-3.0, -1.0], [-3.0, -1.0], color = 'black', linewidth = 2) # use this scale for TNG300-NewSAM where the units of Rdisk and Rhalo are consistent
    ax.set_title('TNG-SIM 0D Dataset \n' + r'RF w $log_{10}all features ', fontsize=14)
    ax.set_xlabel(r'True $log_{10}(Rstar/Rvir)$', fontsize=12)
    ax.set_ylabel(r'Predicted $log_{10}(Rstar/Rvir)$', fontsize=12)
    ...
    ax2.set_title("TNG-SIM 0D Dataset All Morphologies \n Feature importances using Permutation Importances ")
